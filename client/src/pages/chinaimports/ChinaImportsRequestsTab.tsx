...
export default function ChinaImportsRequestsTab({
  detailId,
  onOpenRequest,
  onBackToList,
  onRaiseRequest,
    ...
    <div className="flex flex-col h-full bg-chinaimports-canvas">
      <div className="bg-white px-5 pt-5 pb-3 border-b border-chinaimports-border shrink-0">
        <div className="flex items-center justify-between mb-3">
          <span className="text-[22px] font-black text-chinaimports-ink tracking-tight">Requests</span>
          <button
        ...
        <div className="flex items-center gap-2 bg-chinaimports-canvas rounded-xl px-3 h-[38px] border border-chinaimports-border mb-3">
                ...
                style={
                  active
                    ...
                    : {
                        ...
                        border: "1px solid var(--chinaimports-border)",
                      }
                }
              >
                {opt}
...
function EmptyState({ onRaise, hasFilter }: { onRaise: () => void; hasFilter: boolean }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-12">
      ...
      >
        <Package size={28} color="var(--chinaimports-red)" strokeWidth={1.8} />
      </div>
      <p className="text-[16px] font-bold text-chinaimports-ink mb-1">
        {hasFilter ? "No matching requests" : "No requests yet"}
      </p>
      <p className="text-[12px] text-chinaimports-ink-secondary leading-relaxed max-w-[260px] mb-5">
        {hasFilter
...
function RequestCard({ req, onTap }: { req: SourcingRequest; onTap: () => void }) {
  ...
  return (
    <button
      ...
      onClick={onTap}
      className="bg-white rounded-2xl p-4 border border-chinaimports-border w-full text-left hover:shadow-sm active:scale-[0.99] transition-transform flex gap-3"
    >
      {req.image && (
        <img
          src={req.image}
          alt={req.productName}
          className="w-16 h-16 rounded-xl object-cover border border-chinaimports-border shrink-0 bg-chinaimports-canvas"
          onError={(e) => {
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[14px] font-bold text-chinaimports-ink leading-snug truncate">
          {req.productName}
        </p>
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="text-[10px] text-chinaimports-ink-tertiary">·</span>
          <span className="text-[11px] text-chinaimports-ink-tertiary capitalize">
            {req.shippingSpeed === "air" ? "Air freight" : req.shippingSpeed === "sea" ? "Sea freight" : "—"}
          </span>
          <span className="ml-auto">
            <ChevronRight size={14} color="var(--chinaimports-ink-tertiary)" strokeWidth={2} />
          </span>
        </div>
      </div>
    </button>
    ...
    <div className="flex flex-col h-full bg-chinaimports-canvas">
      <div className="bg-white px-4 pt-4 pb-3 border-b border-chinaimports-border flex items-center gap-3 shrink-0">
        <button
          ...
          onClick={onBack}
          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-chinaimports-canvas active:scale-95 transition-transform"
        >
          <ArrowLeft size={20} color="var(--chinaimports-ink)" strokeWidth={2} />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-[15px] font-bold text-chinaimports-ink">{req.id}</span>
            <span
              className="text-[10px] font-bold px-2 py-0.5 rounded-full"
              style={{ background: color.bg, color: color.fg }}
            >
          ...
          <p className="text-[12px] text-chinaimports-ink-secondary mt-0.5 truncate">{req.productName}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 pb-32 flex flex-col gap-5">
        {req.image && (
          <div className="bg-white rounded-2xl border border-chinaimports-border overflow-hidden">
            <img
              src={req.image}
              alt={req.productName}
              className="w-full h-[200px] object-cover bg-chinaimports-canvas"
              onError={(e) => {
        ...
        <div ref={timelineRef} className="bg-white rounded-2xl p-4 border border-chinaimports-border scroll-mt-4">
          <p className="text-[13px] font-bold text-chinaimports-ink mb-3">Status timeline</p>
          <StatusTimeline status={req.status} />
        </div>

        <div className="bg-white rounded-2xl p-4 border border-chinaimports-border">
          <p className="text-[13px] font-bold text-chinaimports-ink mb-3">Order details</p>
          <DetailRow label="Quantity" value={`${req.quantity} ${req.unit}`} />
          <DetailRow
            label="Shipping"
            value={
              <span className="flex items-center gap-1.5">
                <ShippingIcon size={14} color="var(--chinaimports-red)" strokeWidth={2} />
        ...
        {(req.targetPrice || req.currentPrice || req.purpose) && (
          <div className="bg-white rounded-2xl p-4 border border-chinaimports-border">
            <p className="text-[13px] font-bold text-chinaimports-ink mb-3">Pricing & purpose</p>
              ...
              <DetailRow
                ...
                value={
                  <a
                    ...
                    className="text-chinaimports-red font-semibold underline truncate inline-block max-w-[200px]"
                  >
                    {req.currentPriceLink}
                  </a>
                }
        ...
        {req.agent && (
          <div className="bg-white rounded-2xl p-4 border border-chinaimports-border flex items-center gap-3">
            <div
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] text-chinaimports-ink-tertiary">Assigned to</p>
              <p className="text-[13px] font-bold text-chinaimports-ink truncate">{req.agent.name}</p>
              <p className="text-[11px] text-chinaimports-ink-secondary truncate">{req.agent.role}</p>
            </div>
          </div>
        )}
      </div>

      <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-chinaimports-border px-3 pt-3 pb-4">
...
function ActionButton({
  ...
  return (
    <button
      ...
      style={{
        background: danger ? "transparent" : "var(--chinaimports-canvas)",
        border: `1px solid ${danger ? "rgba(240,59,59,0.3)" : "var(--chinaimports-border)"}`,
        color: danger ? "var(--chinaimports-red)" : "var(--chinaimports-ink)",
      }}
    >
      <Icon
        size={16}
        color={danger ? "var(--chinaimports-red)" : "var(--chinaimports-ink-secondary)"}
        strokeWidth={2}
      />
      {label}
    </button>
...
function DetailRow({
  ...
  return (
    <div className={`py-2 ${multiline ? "flex flex-col gap-1" : "flex items-start justify-between gap-3"}`}>
      <span className="text-[11px] font-semibold text-chinaimports-ink-tertiary uppercase tracking-wider shrink-0">
        {label}
      </span>
      <span className={`text-[13px] text-chinaimports-ink ${multiline ? "leading-relaxed whitespace-pre-wrap" : "text-right"}`}>
        {value}
      </span>
    </div>
  );
          ...
          <div key={s} className="flex items-start gap-3">
            <div className="flex flex-col items-center shrink-0">
              <div
                ...
                style={{
                  background: done
                    ? "var(--chinaimports-red)"
                    : current
                      ? "var(--chinaimports-red-light)"
                      : "var(--chinaimports-canvas)",
                  border: current
                    ? "2px solid var(--chinaimports-red)"
                    : done
                      ? "none"
                      : "1.5px solid var(--chinaimports-border)",
                }}
              >
                {done ? (
                  <CheckCircle2 size={14} color="white" strokeWidth={0} fill="white" />
                ) : current ? (
                  <span className="w-2 h-2 rounded-full" style={{ background: "var(--chinaimports-red)" }} />
                ) : (
                  <Circle size={10} color="var(--chinaimports-border)" strokeWidth={2} fill="none" />
                )}
              </div>
              {!isLast && (
                <div
                  className="w-0.5 h-7"
                  style={{
                    background: done ? "var(--chinaimports-red)" : "var(--chinaimports-border)",
                  }}
                />
              )}
            </div>
            <div className={`pb-4 flex-1 ${isLast ? "pb-0" : ""}`}>
              <p
                ...
                style={{
                  color: done || current ? "var(--chinaimports-ink)" : "var(--chinaimports-ink-tertiary)",
                }}
              >
                {s}
              </p>
              {current && (
                <p className="text-[11px] text-chinaimports-red font-semibold mt-0.5">In progress</p>
              )}
            </div>
          </div>
        );
