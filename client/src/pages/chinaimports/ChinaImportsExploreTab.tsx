...
export const SERVICES: Service[] = [
  ...
  },
  {
    ...
    howItWorks: [
      ...
      "Live tracking updates via the China Imports app",
...
const UPDATE_CARDS: UpdateCard[] = [
  ...
  },
  {
    ...
    body: "Track your goods at container level from departure to arrival directly in the China Imports app.",
  ...
  },
  {
    ...
    tagColor: "#0891B2",
    tagBg: "#CFFAFE",
    title: "China Imports partners with Delhivery for last-mile",
    body: "Our new partnership ensures faster, more reliable domestic delivery across all Indian states.",
    timestamp: "2 weeks ago",
  },
];

export default function ChinaImportsExploreTab() {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-chinaimports-canvas">
      <div className="bg-white px-5 pt-5 pb-3 border-b border-chinaimports-border shrink-0">
        <span className="text-[22px] font-black text-chinaimports-ink tracking-tight">Explore</span>
      </div>
      ...
      <div className="flex gap-2.5 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
        {QUICK_ACTIONS.map(({ id, label, Icon }) => (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-full border border-chinaimports-border bg-white text-[12px] font-semibold text-chinaimports-ink whitespace-nowrap shrink-0 hover:bg-chinaimports-canvas active:scale-95 transition-transform"
          >
            <Icon size={13} color="var(--chinaimports-red)" strokeWidth={2.2} />
            {label}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="px-4 pb-2 shrink-0">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[15px] font-bold text-chinaimports-ink">Our Services</span>
        <span className="text-[12px] font-semibold text-chinaimports-red">See all</span>
      </div>
...
function ServiceCard({ service, onTap }: { service: Service; onTap: () => void }) {
  ...
  return (
    <button
      ...
      onClick={onTap}
      className="bg-white rounded-2xl p-3.5 border border-chinaimports-border flex flex-col gap-2.5 text-left hover:shadow-sm active:scale-[0.98] transition-transform"
    >
      <div className="flex items-start justify-between">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: bgLight }}>
          <Icon size={20} color={color} strokeWidth={1.8} />
        </div>
        <ChevronRight size={14} color="var(--chinaimports-ink-tertiary)" strokeWidth={2} className="mt-1" />
      </div>
      <div>
        <p className="text-[13px] font-semibold text-chinaimports-ink leading-tight">{service.name}</p>
        <p className="text-[11px] text-chinaimports-ink-tertiary mt-0.5 leading-tight">{service.description}</p>
      </div>
    </button>
  );
}
...
function LatestUpdates() {
  return (
    <div className="px-4 pt-2 pb-2 shrink-0">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[15px] font-bold text-chinaimports-ink">Latest Updates</span>
        <span className="text-[12px] font-semibold text-chinaimports-red">See all</span>
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-4 border border-chinaimports-border">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-[11px] text-chinaimports-ink-tertiary">{card.timestamp}</span>
      </div>
      <p className="text-[13px] font-semibold text-chinaimports-ink leading-snug mb-1">{card.title}</p>
      <p className="text-[12px] text-chinaimports-ink-secondary leading-[1.4]">{card.body}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full bg-chinaimports-canvas">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 pb-40">
        ...
        <div className="mb-6">
          <h2 className="text-[15px] font-bold text-chinaimports-ink mb-3">How it works</h2>
          <div className="flex flex-col gap-3">
            {service.howItWorks.map((step, i) => (
              <div key={i} className="flex gap-3 items-start">
                ...
                <p className="text-[13px] text-chinaimports-ink leading-[1.45]">{step}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h2 className="text-[15px] font-bold text-chinaimports-ink mb-3">Why China Imports</h2>
          <div className="flex flex-col gap-2.5">
            {service.whyChinaImports.map((bullet, i) => (
              <div key={i} className="flex gap-2.5 items-start">
                ...
                <p className="text-[13px] text-chinaimports-ink leading-[1.45]">{bullet}</p>
              </div>
            ))}
          </div>
        </div>
